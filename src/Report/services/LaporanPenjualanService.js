...
} = require("../models/LaporanPenjualanModels");
...
const s_getLaporanPenjualan = async (params) => {
  try {
    ...
    for (let i of dataTransaksi) {
      if (i.tipe !== "return") {
        let dataDetailTransaksi = await m_getDetailLaporanTransaksi({
          order_no: i.order_no,
        });
    ...
    data.result[0].result.keuntungan =
      data.result[0].result.net_total - totalModal;
...
const s_exportLaporanPenjualan = async (params) => {
  try {
    return await s_getLaporanPenjualan(params);
